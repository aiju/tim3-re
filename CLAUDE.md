...
- Control type registry — 25 slots, 7-method vtables with null defaults
...
- **Part names** — stored in `INTRFACE.RES`, string group 2000: `getString(2000 + typeId)`
  - `formatPartLabel(part_code, ...)` — formats part name string with escape codes for color/alignment/bitmap
  - `initInterface` — loads INTRFACE.RES string group 2000
